...
/// Feed addresses from an OPML file.
pub fn opml_feeds(text: &str) -> Result<Vec<String>, String> {
    let doc = roxmltree::Document::parse(text).map_err(|_| "That file isn't an OPML list.".to_string())?;
    Ok(doc
        .descendants()
        .filter(|n| n.has_tag_name("outline"))
        .filter_map(|n| n.attribute("xmlUrl").or_else(|| n.attribute("xmlurl")).map(str::to_string))
        .collect())
}
...
mod tests {
    ...
    #[test]
    fn reads_imports() {
        assert_eq!(opml_feeds(r#"<opml><body><outline text="a" xmlUrl="https://f"/></body></opml>"#).unwrap(), vec!["https://f"]);
